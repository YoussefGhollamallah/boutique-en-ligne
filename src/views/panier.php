...
<section class="section ">
    ...
    <aside class="flex space-around ">

        <?php if (!empty($panier)) { ?>
            <?php foreach ($panier as $idProduit => $produit) {
                $checked = isset($produit['checked']) ? $produit['checked'] : false; ?>
                <div class="" id="produit_<?php echo htmlspecialchars($idProduit); ?>">
                    <img class="card_produit_img bg-backgrou" src="assets/images/<?php echo htmlspecialchars($produit['image']); ?>" alt="<?php echo htmlspecialchars($produit['nom']); ?>">
                    <h4>
